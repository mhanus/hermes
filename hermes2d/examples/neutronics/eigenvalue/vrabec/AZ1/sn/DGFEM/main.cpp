...
const bool HERMES_MESH_VISUALIZATION = true;
...
Hermes::MatrixSolverType matrix_solver_type = SOLVER_UMFPACK;
...
int main(int argc, char* args[])
{
  ...
  MaterialProperties::MaterialPropertyMaps matprop(N_GROUPS, data.mat_list);
      ...
      if (!strcmp(args[1], "-sln"))
      {
        ...
        for (int n = 0; n < M; n++)
        {
          ...
          if(VTK_ANG_VISUALIZATION)
          {
            ...
            bool mode_3D = false;
            lin.save_solution_vtk(sol_ext[n], (std::string("sln_") + tostr(n) + std::string(".vtk")).c_str(), "Solution", mode_3D);
          }
        }
...
//  bview.show(spaces[0]);
  ...
  keff_eigenvalue_iteration.set_keff_tol(1e-2*PICARD_TOL);
  //keff_eigenvalue_iteration.set_tolerance(PICARD_TOL);
  keff_eigenvalue_iteration.set_max_allowed_iterations(1000);
  keff_eigenvalue_iteration.use_Anderson_acceleration(false);
  keff_eigenvalue_iteration.set_num_last_vector_used(PICARD_NUM_LAST_ITER_USED);
  keff_eigenvalue_iteration.set_anderson_beta(PICARD_ANDERSON_BETA);
  keff_eigenvalue_iteration.set_verbose_output(true);
  ...
  if (SAVE_SLN_VECTOR)
  {
    ...
    fs << setprecision(16);
    std::copy(keff_eigenvalue_iteration.get_sln_vector(), keff_eigenvalue_iteration.get_sln_vector()+ndof, std::ostream_iterator<double>(fs, "\n"));
  ...
  {
    std::string file = "flux_5_y-R"+tostr(INIT_REF_NUM)+"P"+tostr(P_INIT)+"-S"+tostr(N)+".dat";
    ...
    std::ofstream fs(file.c_str());
    Loggable::Static::info("Saving the scalar flux profile at x=5cm to %s", file.c_str());
