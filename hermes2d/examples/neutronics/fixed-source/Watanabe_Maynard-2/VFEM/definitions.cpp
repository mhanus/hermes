      ...
      for (unsigned int n = 0; n < M; n++)
      {
        if (assemble_all || assemble_L)
        {
          ...
          if (!inflow_boundaries.empty())
            add_matrix_form_surf(new BoundaryStreamingMF(n, gto, G, inflow_boundaries));
        }
    ...
    for ( ; material != matprop.get_materials_list().end(); ++material)
    {
      ...
      for (unsigned int gto = 0; gto < matprop.get_G(); gto++)
      {
        ...
        { 
          ...
          {
            if (*material == "Void")
              add_matrix_form(new VolumetricStreamingAndReactionsMFVoid(regions, n, gto, G));
            else
              add_matrix_form(new VolumetricStreamingAndReactionsMF(regions, n, gto, G, Sigma_t[gto]));
          }
          
          if (!Sigma_sn.empty() && assemble_all && *material != "Void")
            add_vector_form(new VolumetricScatteringSourceVF(odata, regions, n, gto, G, Sigma_sn, Sigma_t[gto]));
          if (src_data[gto] > 0 && (assemble_all || assemble_Q) && *material != "Void") 
...
{
  double result = 0.0;
  double inv_Sigma_t = 1./Sigma_t;
  for (int quad_pt = 0; quad_pt < n; quad_pt++)
    result += wt[quad_pt] * (inv_Sigma_t * ( static_cast<SNWeakForm*>(wf)->calculate_a_dot_v(direction, u->dx[quad_pt], u->dy[quad_pt]) )
                    * ( static_cast<SNWeakForm*>(wf)->calculate_a_dot_v(direction, v->dx[quad_pt], v->dy[quad_pt]) ) + Sigma_t * u->val[quad_pt] * v->val[quad_pt] );
...
double SNWeakForm::VolumetricStreamingAndReactionsMFVoid::value(int n, double *wt, Func<double> *u_ext[], Func<double> *u, Func<double> *v,
                                                  Geom<double> *e, Func<double> **ext) const
{
  double result = 0.0;
  for (int quad_pt = 0; quad_pt < n; quad_pt++)
...
Ord SNWeakForm::VolumetricStreamingAndReactionsMFVoid::ord(int n, double *wt, Func<Ord> *u_ext[], Func<Ord> *u, Func<Ord> *v,
                                                  Geom<Ord> *e, Func<Ord> **ext) const
{ 
  return (u->dx[0]+u->dy[0]) * (v->dx[0]+v->dy[0]);
}
/*
    ...
    for (unsigned int l = 0; l <= L; l++)
    {
      ...
      for (int m = -l; m <= l; m++)
      {
        if ( ((l + m) % 2) == 0 )
        {
          ...
          for (int quad_pt = 0; quad_pt < n; quad_pt++)
          {
            Real tmp = inv_Sigma_t * sln_moment_values_at_quad_pts[quad_pt] * rlm_in_dir * C * Sigma_sn[l][gto][gfrom]
                                                  ...
                                                  Geom<Real> *e, Func<Real> **ext) const
{
  ...
  for (int quad_pt = 0; quad_pt < n; quad_pt++)
  {            
    Real tmp = inv_Sigma_t * Q * ( _wf->calculate_a_dot_v(direction, v->dx[quad_pt], v->dy[quad_pt]) + Sigma_t * v->val[quad_pt] );
