...
private:
  ...
  class VolumetricStreamingAndReactionsMFVoid : protected GenericForm, public MatrixFormVol<double>
  {
  public:
    VolumetricStreamingAndReactionsMFVoid(unsigned int n, unsigned int g, unsigned int G) 
      : GenericForm(n, G), MatrixFormVol<double>(ag.pos(n,g), ag.pos(n,g))
    {
      this->setSymFlag(HERMES_SYM);
    };
    
    VolumetricStreamingAndReactionsMFVoid(const Hermes::vector<std::string>& areas, 
    ...
    MatrixFormVol<double>* clone() const
    {
      return new SNWeakForm::VolumetricStreamingAndReactionsMFVoid(*this);
    }
