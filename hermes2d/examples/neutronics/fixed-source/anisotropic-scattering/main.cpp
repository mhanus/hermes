...
int main(int argc, char* argv[])
{
  ...
  if (STRATEGY >= 0)
  {
    ...
    do 
    {
      ...
      for (unsigned int i = 0; i < N_EQUATIONS; i++)
      {
        const int refine_horizontally = 1;
        Mesh::ReferenceMeshCreator ref_mesh_creator(meshes[i], refine_horizontally);
  ...
  #ifdef USE_SPN    
      Hermes::vector< MeshFunctionSharedPtr<double> >* scalar_fluxes = new Hermes::vector< MeshFunctionSharedPtr<double> >();    
      MomentFilter::get_scalar_fluxes(solutions, scalar_fluxes, N_GROUPS);
  #else // DIFFUSION
      Hermes::vector< MeshFunctionSharedPtr<double> >* scalar_fluxes = &solutions;
  #endif

      double pointwise_flux_err_ref = 0.0;
      for (unsigned int g = 0; g < N_GROUPS; g++)
        for (unsigned int i = 0; i < points_of_interest.size(); i++)
          pointwise_flux_err_ref += Hermes::sqr((scalar_fluxes->at(g)->get_pt_value(points_of_interest[i], 0)->val[0] - ref_solutions[g][i]) / ref_solutions[g][i]);
  ...
  // Output file names.
#ifdef USE_SPN  
  std::string file = std::string("pointwise_flux-sp")+tostr(SPN_ORDER)+std::string(".dat");
  std::string file_err = std::string("pointwise_flux-sp")+tostr(SPN_ORDER)+std::string(".err");
#elif defined(USE_DIFFUSION_WITH_TRANSPORT_CORRECTION)
  ...
  for (unsigned int g = 0; g < N_GROUPS; g++)
  {
    ...
    {
      double pt_flux = scalar_fluxes->at(g)->get_pt_value(points_of_interest[i], 0)->val[0];
